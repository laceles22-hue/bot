...
export default async function HomePage() {
  ...
  if (!data) {
    return <p className="empty">No se ha encontrado el salón. Ejecuta <code>npm run db:seed</code>.</p>;
  }
  ...
  const dayLabel = new Date().toLocaleDateString("es-ES", { weekday: "long", day: "numeric", month: "long", year: "numeric" });
  ...
  return (
    <>
                  ...
                  {appointmentsToday.map((a) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ left: `${left}%`, width: `${width}%`, top: `${row * 26}px`, background: employeeColorById.get(a.employeeId) }}
                      >
          ...
          <div className="mock">
            {!clientWithAllergy ? (
              ...
              <div className="client-row">
                <div className="avatar">{clientWithAllergy.firstName[0]}{clientWithAllergy.lastName[0]}</div>
                <div>
                  <div className="client-name">{clientWithAllergy.firstName} {clientWithAllergy.lastName}</div>
              ...
              commissionRanking.map((c, i) => (
                <div className="rank-row" key={c.employee!.id}>
                  ...
                  <span className="rank-bar-bg">
                    <span className="rank-bar" style={{ width: `${(c.amount / maxCommission) * 100}%`, background: SERIES_COLORS[i % SERIES_COLORS.length] }} />
                  </span>
      ...
      <footer className="page-foot">
        Salon Manager — datos leídos en directo desde PostgreSQL vía Prisma.
      </footer>
    </>
  );
}
