...
export async function getDashboardData() {
  // Single-tenant lookup until authentication (per-tenant sessions) is built.
  ...
  const [
    ...
    prisma.appointment.findMany({
      where: { tenantId: tenant.id, startTime: { gte: todayStart, lte: todayEnd } },
      include: { client: true, employee: true, services: { include: { service: true } } },
      orderBy: { startTime: "asc" },
    }),
    prisma.appointment.count({
      where: { tenantId: tenant.id, startTime: { gte: yesterdayStart, lte: yesterdayEnd } },
    }),
    prisma.invoice.findMany({
      where: { tenantId: tenant.id, issuedAt: { gte: todayStart, lte: todayEnd }, status: "ISSUED" },
    }),
    prisma.user.count({ where: { tenantId: tenant.id, role: "STYLIST", active: true } }),
    prisma.client.findFirst({
      where: { tenantId: tenant.id, allergies: { some: {} } },
    ...
    prisma.commissionRecord.groupBy({
      by: ["employeeId"],
      where: { tenantId: tenant.id, periodMonth },
      _sum: { amount: true },
    }),
    prisma.invoice.findMany({
      where: { tenantId: tenant.id },
